...
namespace
{
static const char* PLUGIN_NAME {"customPrpool"};
...
class CustomPrpoolPlugin : public IPluginV2DynamicExt {
public:
    /*
    ...
    */
    CustomPrpoolPlugin() = delete; //默认构造函数，一般直接delete
    CustomPrpoolPlugin(const std::string &name, int pooled_height, int pooled_width, float spatial_scale);  //parse时候用的构造函数
    CustomPrpoolPlugin(const std::string &name, const void* buffer, size_t length); //clone以及反序列化的时候用的构造函数
...
class CustomPrpoolPluginCreator : public IPluginCreator {
public:
    CustomPrpoolPluginCreator();  //初始化mFC以及mAttrs
    ~CustomPrpoolPluginCreator();
